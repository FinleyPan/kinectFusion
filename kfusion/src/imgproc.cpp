...
void kfusion::cuda::computeDists(const Depth& depth, Dists& dists, const Intr& intr)
{
    ...
    device::compute_dists(depth, dists, make_float2(intr.fx, intr.fy), make_float2(intr.cx, intr.cy)); //注意这里的类型Dists与device::compute_dists()中的Dists类型不一致！所以会调用operator PtrStepSz<T>()
}

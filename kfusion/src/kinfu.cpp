...
kfusion::KinFuParams kfusion::KinFuParams::default_params()
{
    ...
    p.volume_pose = Affine3f().translate(Vec3f(-p.volume_size[0]/2, -p.volume_size[1]/2, 0.5f));//按行存储，体素空间的初始位姿

    p.bilateral_sigma_depth = 0.04f;  //meter，双边滤波的两个sigma
...
void kfusion::KinFu::allocate_buffers()
{
    ...
    for(int i = 0; i < LEVELS; ++i)
    {
        ...
        cols /= 2; //随着金字塔的上升，图像尺寸变为原尺寸的四分之一
        rows /= 2;
    }
...
bool kfusion::KinFu::operator()(const kfusion::cuda::Depth& depth, const kfusion::cuda::Image& /*image*/)
{
    ...
    const int LEVELS = icp_->getUsedLevelsNum(); //返回迭代次数为0时对应的索引

...
{
    Context context; //OpenNI的工作空间
    ScriptNode scriptNode; //从文件或字符串中载入一个XML脚本，然后可以运行这个脚本来创建一个产品图(production graph)。它会引用图中的所有产品节点。
    DepthGenerator depth; //从设备中获取输入，产生深度图，深度图中的每个像素值表示到传感器平面的距离(单位为毫米)；还可以获取设备支持的最大深度和FOV等信息
    ImageGenerator image; //产生不同格式的RGB图
    ProductionNode node; //所有产品节点(production node)和产生器节点(generator node)的基类
...
void kfusion::OpenNISource::getParams ()
{
    ...
    rc = impl_->depth.GetRealProperty ( "ZPPS", pixelSize );  // in mm,零平面像素大小
    if (rc != XN_STATUS_OK)
    {
    ...
    XnUInt64 depth_focal_length_SXGA_mm;   //in mm,零平面距离
    ...
    XnDouble baseline_local; //IR projector(发射)和IR camera(接收)间的基线距离(单位为cm)
    ...
    XnUInt64 shadow_value_local; //如果深度图中的像素存在阴影(in shadow),则该像素的深度值使用该值来替代
    ...
    XnUInt64 no_sample_value_local; //如果该像素深度值无法获取，则该像素的深度值使用该值来替代
    ...
    //focal length from mm -> pixels (valid for 1280x1024) //将焦距的度量单位转换为像素

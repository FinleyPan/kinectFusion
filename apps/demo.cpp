...
struct KinFuApp
{
    static void KeyboardCallback(const cv::viz::KeyboardEvent& event, void* pthis)
    {
		...
		//需要注意的是每次按一个键时，其实触发了两个事件，先触发KEY_DOWN再触发KEY_UP；因此这里可理解为一旦键按下，且键位为t或i则执行某些操作；键位回弹该函数什么都不做
    ...
    bool execute()
    {
        ...
        for (int i = 0; !exit_ && !viz.wasStopped(); ++i)
        {
            ...
            {
                SampledScopeTime fps(time_ms); (void)fps; //(void)fps的作用是消除编译器警告fps未使用
...
int main (int argc, char* argv[])
{
    ...
    //capture.open (0);
	capture.open("F:\\RGBDdata\\fountain.oni");
